...
$FlagName= $_GET['flag'];
$SupportGroup = $_GET['SupportGroup'];
...
$smarty->assign("PageTitle", "Processing Tickets assigned to " . $SupportGroup);
...
// Get Amount of processing filtered tickets
// list Processing Tickets
$query="Select
count(raynet_glpi_tickets.id)
From
raynet_glpi_tickets 
left JOIN glpi_entities On raynet_glpi_tickets.entities_id = glpi_entities.id 
left Join glpi_tickets_users On glpi_tickets_users.tickets_id = raynet_glpi_tickets.id
    left Join glpi_users On glpi_tickets_users.users_id = glpi_users.id
  left join glpi_groups_tickets ON glpi_groups_tickets.tickets_id = raynet_glpi_tickets.id
  left join glpi_groups ON glpi_groups.id = glpi_groups_tickets.groups_id
Where
raynet_glpi_tickets.status in (2,3) And
glpi_entities.completename Like '%". $Entity ."%' And
glpi_tickets_users.TYPE = 1 and
  glpi_groups_tickets.`type` = 2 and
  glpi_groups.name like '" . $SupportGroup . "%'";


$data = mysqli_query($link, $query);
$res= mysqli_fetch_array($data);
$smarty->assign("ProcessingFiltered",$res[0]);
...
glpi_tickets_users.TYPE = 1 and
  glpi_groups_tickets.`type` = 2 and
  glpi_groups.name Like '" . $SupportGroup . "'
  order by raynet_glpi_tickets.id ASC";
...
$smarty->display('Raynet_Tickets_Filtered.tpl');
